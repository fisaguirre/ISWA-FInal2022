...
describe('Address e2e test', () => {
  ...
  const addressSample = { postcode: 'Officer', country: 'Ba' };
  ...
  describe('new Address page', () => {
    ...
    it('should create an instance of Address', () => {
      ...
      cy.get(`[data-cy="address2"]`).type('Union').should('have.value', 'Union');

      cy.get(`[data-cy="city"]`).type('Kiraport').should('have.value', 'Kiraport');

      cy.get(`[data-cy="postcode"]`).type('Forward Me').should('have.value', 'Forward Me');

      cy.get(`[data-cy="country"]`).type('Un').should('have.value', 'Un');
